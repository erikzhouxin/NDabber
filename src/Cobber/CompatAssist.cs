...
namespace System.Data.Cobber
{
    ...
    public static class CompatAssist
    {
        ...
        /// <returns></returns>
        public static IEnumerable<T> GetCustomAttributes<T>(this MemberInfo member, bool isHerit = false) where T : Attribute
        {
            foreach (var attr in member.GetCustomAttributes(typeof(T), isHerit))
            {
                if (attr is T real) { yield return real; }
            }
        }
        /// <summary>
        /// 获取自定义属性
        ...
        /// <param name="inherit"></param>
        /// <returns></returns>
        public static IEnumerable<T> GetCustomAttributes<T>(this PropertyInfo prop, bool inherit = false) where T : Attribute
        {
            foreach (var item in prop.GetCustomAttributes(inherit))
            {
