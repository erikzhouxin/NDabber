...
{
    internal static class HopperCompat
    {
        public static readonly TimeSpan InfiniteTimeSpan = new TimeSpan(0, 0, 0, 0, -1);
#if NET40
        public static bool TryAddWithoutValidation(this HttpContentHeaders header, string name, string value)
        {
            if (value == null) { value = string.Empty; }
            header.Add(name, value);
            return true;
        }
        public static bool TryAddWithoutValidation(this HttpContentHeaders header, string name, string[] values)
        {
        ...
        public static bool TryAddWithoutValidation(this HttpHeaders header, string name, string[] values)
        {
            if (values == null) { return true; }
            foreach (var val in values)
            {
        ...
        public static bool TryGetValues(this HttpContentHeaders header, string name, out IEnumerable<string> values)
        {
            try
            {
                values = header.GetValues(name);
                return true;
            }
            catch (Exception)
            {
...
namespace System.Threading
{
    ...
    public sealed class AsyncLocal<T> : IAsyncLocal
    {
        ...
        {
            get => (T)_value;
            set => SetLocalValue(this, value, _valueChangedHandler is not null);
        }
